        ...
        if (detailRecord.visible) {
          ...
          detailRecord.optionsList.forEach((item, index) => {
            ...
            drawStuList(stuRequest).then((res: any) => {
              (fItem.count = res.total), (fItem.choer = res.rows);
  ...
  <div class="bg-gray-500/8 p-3 h-screen">
    <div>
      <div class="text-left ml-3">
        ...
        <span
          class="bg-hex-30B648 rounded-lg text-white text-xs py-0.5 px-2 ml-2"
          >1 票</span
        >
      </div>
      <div
        class="text-left mt-2 p-5 text-sm bg-white rounded border-t-2 border-hex-30B648"
      >
      ...
      <van-tabs color="rgb(40,182,72)" background="rgb(243,244,245)">
        <van-tab>
          ...
          <div class="mt-3" v-if="detailRecord.visible">
            <div v-for="item in cho_list" :key="item">
              <div
                class="bg-white mt-3 rounded flex justify-between p-3 text-14px"
              >
                <div>
                  <span>抽中：</span>
                ...
                <div>
                  <span
                    class="bg-hex-30B648 rounded-lg text-white text-xs py-0.5 px-2 mr-3"
                    >{{ item.count }} 票</span
                  >
              ...
              <div
                v-show="item.isShow"
                class="bg-white rounded border border-hex-ccc mt-3 px-5"
              >
                <van-list>
                  <ul
                    class="flex justify-around border-b border-hex-ccc py-3 text-sm"
                  >
                    <span class="flex-1"
                      >学号/工号<van-icon name="sort"
                    /></span>
                    <span class="flex-1">姓名</span>
                    <span class="flex-1">时间</span>
                  </ul>
                  <ul
                    class="flex justify-around border-b border-hex-ccc py-3 text-sm items-center"
                    v-for="choerItem in item.choer"
                    :key="choerItem"
                  >
          ...
          <div
            class="mt-3 bg-white rounded border border-hex-ccc px-5 py-2"
            v-if="!detailRecord.visible"
          >
            <van-list>
              <ul
                class="flex justify-around border-b border-hex-ccc py-3 text-sm"
        ...
        <van-tab>
          ...
          <div class="mt-3 bg-white rounded border border-hex-ccc px-5 py-2">
            <van-list>
              ...
              <ul
                class="flex justify-around border-b border-hex-ccc py-3 text-sm"
                v-for="item in no_list"
                :key="item"
              >
                <span class="flex-1">{{ item.num }}</span>
                <span class="flex-1">{{ item.name }}</span>
                <span class="flex-1" style="color: rgb(0, 102, 204)">{{
                  item.status
                }}</span>
              </ul>
              <ul class="text-x py-3">

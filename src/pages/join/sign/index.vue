...
const signData: SignData = reactive({
  ...
  tittleText: computed(() => {
    if (!pattern.test(signData.signCode)) return "请输入4位有效的签到码*^_^*";
    else if (
      pattern.test(signData.signCode) &&
      signData.code !== signData.signCode
    )
...
<template>
  <div class="bg-true-gray-50 py-5 px-4">
    <div
      class="bg-cyan-100 rounded-sm border-1 border-blue-300 p-2 text-xs"
      style="background-color: #dff7fa; color: #003399"
    >
      {{ signData.tittleText }}
    </div>
    <div class="my-6 flex justify-center">
      <van-cell-group
        v-if="!signData.isCheckCode"
        inset
        class="border rounded-0.5 w-25"
        style="border-color: #999999"
      >
        <van-field
          v-model="signData.signCode"
          placeholder="输入签到码"
          input-align="center"
          class="p-1"
          maxlength="4"
    ...
    <div class="flex justify-center">
      <van-button
        ...
        type="primary"
        class="w-30 font-600"
        :disabled="!(signData.code === signData.signCode)"
        color="green"
        @click="
          !signData.isCheckCode ? checkCode() : router.push('/join/sign/detail')
        "
      >
        {{ signData.buttonText }}
      </van-button>

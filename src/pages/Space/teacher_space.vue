...
const choose = ref();
const onConfirm = () => {
  if (choose.value === "no") classShow.value = true;
  else if (choose.value === "yes") classSpaceshow.value = true;
};
  ...
  <div class="p-3 bg-gray-500/8">
    <div class="flex justify-between items-center">
      <span class="px-3 py-1 flex items-center bg-hex-fff rounded w-xs">
        <span class="mr-3 text-2xl" style="color: rgb(156, 163, 175)"
          ><van-icon class="font-700" name="search"
        /></span>
        <input placeholder="请输入要搜索的空间" />
      </span>
      <span class="text-xl" @click="changeShow">
        <span
          class="color-hex-59B476 border-3 border-hex-59B476 rounded font-bold px-0.2 mr-2"
      ...
      <div class="mx-4 border-t border-b border-hex-aaa mt-3 flex justify-left">
        ...
        <span class="flex p-2">
          <van-radio-group v-model="choose" direction="horizontal">
            <van-radio name="yes" class="px-2"> 是 </van-radio>
            <van-radio name="no" class="px-2"> 否 </van-radio>
          </van-radio-group>
        </span>
      </div>
    </van-dialog>
        ...
        <div class="text-sm p-1">
          ...
          <van-checkbox-group ref="checkboxGroup" v-model="class_checked">
            <van-checkbox
              ...
              shape="square"
            >
              <div class="text-sm p-1">班级：{{ item.class }}</div>
            </van-checkbox>
          </van-checkbox-group>
        </div>
      </div>

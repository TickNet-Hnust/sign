...
const getsignSpaceList = () => {
  ...
  signSpaceList(request.value)
    .then((res: any) => {
      if (res.code === 200) spaceList.value = spaceList.value.concat(res.rows);
    ...
    addAFTFSpace(addAFTFSpaceData).then((res) => {
      ...
      if (res.code === 200) {
        ...
        if (res.msg === "只能包括逗号、下划线、汉字、数字、字母！") {
          ...
          Notify({
            type: "warning",
            message: "只能包括逗号、下划线、汉字、数字、字母！",
          });
        }
      }
    });
  } else if (isJoin.value === true) {
    // 加入空间
  ...
  <div class="p-3 bg-gray-500/8 min-h-screen">
    ...
    >
      <div class="mt-5 px-10">
        ...
        <van-field
          v-model.trim="addAFTFSpaceData.code"
    ...
    >
      <div class="mt-5 px-5">
        <div class="text-14px text-hex-999">请输入要创建的空间名称</div>
        <van-field
          v-model.trim="addAFTFSpaceData.spaceName"
          class="border-b border-hex-ccc mb-3"
        />
        <div class="text-14px text-hex-999">
    ...
    <div class="mt-8 border-1 border-hex-DEDEDE bg-hex-fff rounded py-3 px-5">
      ...
      <van-list>
        ...
        <div
          ...
          class="text-sm text-hex-aaa mb-5 px-5 mt-10"
        >
          空空如也~
        </div>
        <van-list />

  ...
  <div class="p-3 bg-gray-500/8 h-screen">
    <div class="flex justify-between items-center">
      <span class="py-1 flex items-center bg-hex-fff rounded w-xs">
        <van-search
          v-model.trim="searchJoindata.spaceName"
          class="w-full"
          show-action
    ...
    <div class="mt-8 border-1 border-hex-DEDEDE bg-hex-fff rounded py-3 px-5">
      <van-tabs
        ...
        color="rgb(40,182,72)"
        @rendered="getManageSpace()"
      >
        <van-tab title="我管理的">
          <van-list>
            <ul
              v-for="item in manage_space_list"
              :key="item"
            ...
            <div v-if="showManage" class="text-sm text-hex-aaa mb-5 px-5 mt-10">
              暂时还没有创建空间，快去创建一个属于自己的空间叭(*^▽^*)
            </div>
          </van-list>
        </van-tab>
        <van-tab title="我参与的">
          <van-list>
            ...
            <div v-if="showJoin" class="text-sm text-hex-aaa mb-5 px-5 mt-10">
              暂时还没有参与的空间，快去参加一个空间叭(*^▽^*)
            </div>
          </van-list>
        </van-tab>
      </van-tabs>

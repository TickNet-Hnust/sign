...
onMounted(() => {
  getVote(voteId).then((res) => {
    ...
    props.endTime = `${voteData.endTime.split(" ")[1].split(":")[0]}:${
      voteData.endTime.split(" ")[1].split(":")[1]
    }`;
    console.warn(props);
    let i = 1;
    voteData.option.pop();
    ...
    if (res.data.presentChoices !== null) {
      res.data.presentChoices.forEach((key: string) => {
        for (const item of voteData.option) {
          if (item.optionValue === key) voteData.optionChecked.push(item.id);
        }
      });
    }
  });
